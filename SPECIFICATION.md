...
JSON Comparison Engine	DONE — v1.1	Built-in diff tool ported from json810Compare. Profile-driven comparison engine in `pyedi_core/comparator/`, `pyedi compare` CLI, portal `/compare` page. SQLite storage with field_crosswalk table for runtime severity overrides + amount_variance. 11 profiles (810, 850, 855, 856, 860, 820, CSV, cXML, bevager_810, darden_asbn, retalix_p_i_invo). Flat file compare mode (`_compare_flat_dict`) for structured JSON. Scaffold-rules CLI generates compare rules from compiled schemas. Trading partners onboarded: Bevager (2026-03-26), Darden ASBN (2026-03-27), Retalix PI Invoice (2026-04-02). All 11 sqlLiteReport.md improvement tasks implemented. See `rulesApproach.MD` for 3-tier rule architecture details.
