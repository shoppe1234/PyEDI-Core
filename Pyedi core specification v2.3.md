...
The primary goal of v2.3 is to take normalized JSON outputs from diverse source formats (EDI, CSV, Fixed-Length) and compare them reliably. Every JSON output carries a consistent envelope (`schema_version`, `source_system_id`, `transaction_type`, `batch_id`, `correlation_id`, `processed_at`, `source_file`) enabling deterministic comparison without ambiguity.
...
│   └── fixed_length_handler.py # Fixed-length positional file driver
├── schemas/
...
•	On every CSV or Fixed-Length file detection, check if a compiled YAML exists in `./schemas/compiled/`
...
Parsing rules: extract field names, map types (String→string, Decimal→float, Integer→integer), identify structure (Header / Details / Summary). Output is a standards-compliant YAML map.
...
•	Routing: Matches file directory against `csv_schema_registry` in config
...
•	Detection: Matches file directory against `fixed_length_schema_registry` in config
•	**Schema-Driven Parsing**: No hardcoded field lengths or offsets in Python. All parsing logic is driven by the compiled YAML schema generated from the source DSL.
•	**Multi-Document Segmentation**: Automatically splits files containing multiple invoices by detecting the "invoice boundary" record defined in the schema (e.g., `OTpidGroup`).
•	**Type Conversion**: Handles implied decimals (e.g., "001250" -> 12.50) via schema definitions.
•	**Routing**: Uses `fixed_length_schema_registry` to resolve the correct schema based on the inbound directory path.
•	Triggers `schema_compiler.py` if the compiled YAML is missing or outdated.
...
|---|---|---|---|
| Phase 1 | Core Engine | All core/ modules, all drivers (X12, CSV, XML, Fixed), config.yaml + Pydantic validation, CLI via main.py, .processed manifest, dead letter queue, dry-run mode | Weeks 1–3 |
...
•	10. drivers/fixed_length_handler.py — needs schema_compiler, mapper, error_handler
...
•	All drivers process their respective fixture files successfully
