...
## CONSOLIDATED DEVELOPER SPECIFICATION & ROADMAP
**Version:** 2.3
...
•   Detection: `inbound_dir` match in `fixed_length_schema_registry`.
•   Uses a compiled YAML schema (generated from DSL by `schema_compiler.py`) to parse positional fields.
•   No hardcoded positions or record IDs in Python code — completely schema-driven.
•   Supports multi-document files via schema-defined invoice boundary triggers.
•   Handles type conversion (integer, decimal, implied_decimal) during parsing.
•   Reconstructs hierarchical structure (Header/Lines/Summary) before mapping.
...
| `GET /status/{id}` | GET | Look up processing status from manifest. |
...
|---|---|---|---|
| Phase 1 | Core Engine | All core/ modules, all 4 drivers, config.yaml + Pydantic validation, CLI via main.py, .processed manifest, dead letter queue, dry-run mode | Weeks 1–3 |
...
•	All four drivers process their respective fixture files successfully
